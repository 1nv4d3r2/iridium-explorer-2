...
def iridium_init_rpc_parameters(chain):
    
    if readconf.is_missing(cfg.settings[chain],'datadir'):
        cfg.settings[chain]["datadir"]="~/.iridium"
    ...
    if readconf.is_missing(cfg.settings[chain],'rpcport'):
        ...
        if not utils.file_exists(params_file):
            utils.print_error("Couldn't find Iridium directory " + datadir)            
            return False
    ...
    conf_file=datadir + "iridium.conf"     
...
class MCEChain:
    ...
    def initialize(self):
        ...
        self.config['iridium-url']=url
        self.config['iridium-headers']=headers
...
#        print(self.config)        
    ...
    def request(self, method, params=[]):
        ...
        headers=self.config['iridium-headers'].copy()
        headers["Content-Length"] = str(len(payload))
        
        try:
    #        req = requests.post(cfg.iridium_url, data=payload, headers=headers)
        
            data = str(payload)
            data = data.encode('utf-8')
            ureq =  request.Request(self.config['iridium-url'], data=data)
            ...
            error_str="Iridium is not running: " + str(e.reason)
            req_json={
                'result': None,
                'error' : error_str,
...
#            print("C")
#            error_str="Iridium is not running: " + str(error)
#            print(str(error))
#            req_json={
#                'result': None,
#                'error' : error_str,
...
#        req_json=json.loads(resp.decode('utf-8'))
        ...
        if req_json is None:
            error_str="Iridium connection error"
            req_json={
                'result': None,
                'error' : error_str,
                'connection-error' : True
